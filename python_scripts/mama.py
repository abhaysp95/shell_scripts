...
def generate_manifest(archive_path, storage_name, compression_algo="zstd", compression_level=None, encryption_enabled=False, encryption_method=None):
    archive_size = os.path.getsize(archive_path)
    archive_checksum = sha256_checksum(archive_path)
    archive_name = os.path.basename(archive_path)
    creation_date = datetime.now(timezone.utc).isoformat() + "Z"

    files_info = list_tar_zst_contents(archive_path)
...
def main():
    ...
    manifest = generate_manifest(archive_path, storage_name, compression_level=compression_level)
